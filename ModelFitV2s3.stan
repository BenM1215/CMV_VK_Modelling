...
parameters {
  ...
  real <lower = 0> mu[6];             // parameter means
  real <lower = 0> phi[6, nSubjects];  // 1 vector of parameters per patient
...
transformed parameters {
    ...
    for (subject in 1:nSubjects) {    // loop over patients
      ...
      ind_theta[1, subject] = lambda;
      ind_theta[2, subject] = d;
      //ind_theta[3, subject] = k;
      ind_theta[4, subject] = delta;
      ind_theta[5, subject] = rho;
      ind_theta[6, subject] = c;
...
model { 
  // *** Literature priors ***
  //mu[1] ~ normal(10000, 2);     // lambda
  //mu[2] ~ normal(0.03, 0.01);   // d
  //mu[3] ~ normal(2.6e-8, 0.01); // k
  //mu[4] ~ normal(0.3, 0.2);     // delta
  //mu[5] ~ normal(400, 0.2);     // rho
  //mu[6] ~ normal(5, 0.2);       // c
  ...
  mu[1] ~ normal(0, 100);       // lambda
  mu[2] ~ normal(0, 100);       // d
  mu[3] ~ normal(0, 100);       // k
  mu[4] ~ normal(0, 100);       // delta
  mu[5] ~ normal(0, 100);       // rho
  mu[6] ~ normal(0, 100);       // c
  ...
  phi[2,] ~ normal(mu[2], 0.5);
  phi[3,] ~ normal(mu[3], 0.5);
  phi[4,] ~ normal(mu[4], 0.5);
  phi[5,] ~ normal(mu[5], 0.5);
  phi[6,] ~ normal(mu[6], 0.5);
