functions{
  real[] ode(     int time,    // time
      ...
      real drugs[min(x_i)];          // data - drug
      real t[min(x_i)];          // data - time
      
      //x_i contains time points when drugs were administered
      ...
      //to test
      // pass int of max size (index) loop to index max access time and treatment - check == 1
      trigger = 0;
...
parameters {
  ...
  real <lower = 0> drugE_phi[nSubjects];
...
transformed parameters {
    ...
    for (subject in 1:nSubjects) {    // loop over patients
      ...
      real x_r[2, s[subject]];        // stores x_r data
      
      ind_theta[, subject] = phi[, subject];     // individual level effects
      
      x_r[1,] = drugs[position:position+s[subject]-1];
      x_r[2,] = time[position:position+s[subject]-1];
      
      //print(drugs[position:position+s[subject]-1])
      //yhat = 
